...
export function PhoneFrame({
  ...
  return (
    <div className={clsx('relative aspect-366/729', className)} {...props}>
      ...
      <div className="absolute top-[calc(23/729*100%)] left-[calc(23/366*100%)] grid h-[calc(686/729*100%)] w-[calc(318/366*100%)] transform grid-cols-1 overflow-hidden bg-white">
        {children}
      </div>
...
export function Screenshot({
  ...
  className,
  ...props
}: React.ComponentPropsWithRef<'img'> & { name: string; alt: string }) {
  return (
    <img
      src={`${basePath}/screenshots/${name}.png`}
      alt={alt}
      className={clsx('col-start-1 row-start-1 h-full w-full object-cover object-top', className)}
      {...props}
    />
  );
}
