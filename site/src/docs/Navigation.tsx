...
import { useIsInsideMobileNavigation } from '@/docs/MobileNavigation';
import { useSectionStore } from '@/docs/SectionProvider';
import { remToPx } from '@/lib/remToPx';
import { type NavGroup, navigation, repoUrl, withoutTrailingSlash } from '@/lib/site';
...
function NavigationGroup({ group, className }: { group: NavGroup; className?: string }) {
  ...
  let [pathname, sections] = useInitialValue(
    [withoutTrailingSlash(usePathname()), useSectionStore((s) => s.sections)],
                ...
                {link.href === pathname && sections.length > 0 && (
                  <motion.ul
                    ...
                    {sections.map((section) => (
                      <li key={section.id}>
                        <NavLink href={`${link.href}#${section.id}`} isAnchorLink>
                          {section.title}
                        </NavLink>
                      </li>
                    ))}
  ...
  return (
    <nav {...props}>
      <ul role="list">
        ...
        <TopLevelNavItem href="/#download">Download</TopLevelNavItem>
        <TopLevelNavItem href={repoUrl}>GitHub</TopLevelNavItem>
        ...
        <li className="sticky bottom-0 z-10 mt-6 min-[416px]:hidden">
          <Button href="/#download" variant="filled" className="w-full">
            Download
          </Button>
        </li>
      </ul>
