...
export function HistoryStatus({ protocol }: { protocol?: string }) {
  ...
  const active = [...fetching, ...connecting.filter(([id]) => !fetching.some(([key]) => key === id))];
  ...
  return (
    <View className="gap-1 px-gutter py-3" accessibilityLiveRegion="polite">
      {active.length > 0 ? (
        <View className="flex-row items-center gap-2" accessible accessibilityLabel={`${fetching.length ? 'Fetching history' : 'Connecting'}: ${active.map(([id]) => protocolLabel(id)).join(', ')}`}>
      ...
      {failed.length > 0 ? (
        <Pressable accessibilityRole="button" accessibilityLabel={`Retry fetching history. ${details(failed)}`} onPress={() => { for (const [id] of failed) void syncProtocol(id); }} className="py-1">
          <Text variant="caption">
            {details(failed)} · Retry
          </Text>
        </Pressable>
      ) : null}
      {partial.length > 0 ? (
        <Pressable accessibilityRole="button" accessibilityLabel={`Retry unavailable relays. ${details(partial)}`} onPress={() => { for (const [id] of partial) void syncProtocol(id); }} className="py-1">
          <Text variant="caption">
            {details(partial)} · Retry
          </Text>
        </Pressable>
      ) : null}
    </View>
