...
export function walletErrorMessage(
  ...
  const text = [
    ...
    ...causes.map((entry) => entry instanceof BaseError
      ? `${entry.shortMessage}\n${entry.details ?? ''}`
      : typeof entry.message === 'string' ? entry.message : ''),
  ].join('\n');
  const reverted = causes.some((entry) =>
  ...
  if (causes.some((entry) => entry instanceof LimitExceededRpcError || entry.status === 429 || entry.code === 429 || entry.code === -32005)
    || /rate[ -]?limit|too many requests|(?:\b(?:HTTP(?: error| status)?|status:?)\s+|\bfailed \(|\breturned )429\b/i.test(text)) {
  ...
  if (causes.some((entry) => entry instanceof InsufficientFundsError)
    || (!reverted && /out[ _-]?of[ _-]?funds|insufficient (?:funds|balance|lamports)|not enough (?:funds|balance|lamports)|exceeds (?:the )?(?:transaction sender account )?balance/i.test(text))) {
  ...
  if (causes.some((entry) => entry.code === 4902)
    || /unsupported (?:chain|network)|(?:chain|network)[^\n]*\b(?:not supported|not available in this app|not configured)\b/i.test(text)) {
  ...
  if (causes.some((entry) => entry instanceof UserRejectedRequestError || entry.code === 4001)
  ...
  if (/invalid (?:recipient|address)|(?:recipient|address)[^\n]*(?:is not valid|is invalid)|is not (?:an? )?(?:\w+ )?address/i.test(text)) {
  ...
  if (causes.some((entry) => entry.status === 401 || entry.status === 403)
  ...
  if (causes.some((entry) => entry instanceof TimeoutError || entry.code === 'ETIMEDOUT')
  ...
  if (causes.some((entry) => entry instanceof HttpRequestError || entry.code === 4900 || entry.code === 4901 || entry.name === 'AbortError'
      || /^(?:ECONNRESET|ECONNREFUSED|ENOTFOUND|ENETUNREACH)$/.test(String(entry.code)))
    || /fetch failed|failed to fetch|network request failed|network error|offline|connection (?:closed|failed|lost)|socket (?:closed|hang up)|HTTP request failed|\bHTTP(?: error| status)?\s*\d{3}\b|\b(?:failed \(|returned )(?:408|5\d{2})/i.test(text)) {
  ...
  const diagnostic = causes.some((entry) => entry instanceof BaseError || typeof entry.code === 'number')
    || /(?:https?|wss?):\/\/|\b(?:rpc|json|stack|authorization|bearer|api[ _-]?key|password|secret|credential)\b|request body|\b(?:url|headers|details|version):|\bviem@|\bat \S+\s*\(|transaction creation failed/i.test(text);
