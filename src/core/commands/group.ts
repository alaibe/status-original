...
function groupGuard(conversationId: string):
    | { ok: true; conversation: Conversation; selfRole: GroupRole }
...
async function membersCard(conversationId: string, note?: string) {
  ...
  const names = await resolveNames(protocol, members.map((m) => m.id));
