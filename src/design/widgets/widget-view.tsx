...
/** A field whose options sit behind a sheet, for lists too long to wear as chips. */
  ...
  return (
    <FieldShell label={label} hint={field.hint ? fillText(field.hint, display) : undefined}>
      ...
      <ActionSheet
        ...
        // Built only while open: the form above re-renders on every keystroke.
        actions={
          picking
            ? options.map((option) => ({
                label: option.label,
...
function FormWidget({
  ...
  // Corrected on the way out rather than written back, so what is rendered,
  // what is checked for completeness and what is submitted are one thing.
